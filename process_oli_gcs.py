...
def strips_tif(infolder,outfolder):
    #strips the tiff from gcs prior to the process of l2gen seadas

    if not os.path.exists(outfolder):
        os.mkdir(outfolder)

    #strips the tiff using geotifcp -s
    all_files = glob.glob(os.path.join(infolder,'*.TIF'))

    for file in all_files:
        outfile = os.path.basename(file)
        cmd = 'geotifcp -s '+ file + ' '+ os.path.join(outfolder,outfile)
        print(cmd)
        os.system(cmd)

    # copy the MLT.txt and ANG.txt to the outfolder
    for file in glob.glob(os.path.join(infolder,'*.txt')):
        outfile = os.path.basename(file)
        cmd = 'cp '+ file + ' '+ os.path.join(outfolder,outfile)
        print(cmd)
        os.system(cmd)
...
def run_process(l1a_file, working_path, out_path):
    ...
    #
    ancfile = working_path + os.path.basename(l1a_file) + '.anc'
    if status_code != 0:
...
if __name__ == '__main__':
    ...
    source = "/Users/zhigang/Desktop/SateliteData/"
    ...
    all_files = glob.glob(source + 'LC08_L1TP*T1')
